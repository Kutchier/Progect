...
window.Combat3D = (() => {
  ...
  function buildLightSource(x, y, z, themeName, theme) {
    ...
    if (themeName === 'temple' || themeName === 'throne') {
      ...
      // Standing lantern
      g.add(cyl(0.04, 0.04, 0.7, 8, mat(0x303030), 0, 0.4, 0));
      g.add(box(0.2, 0.24, 0.2, mat(0x282828), 0, 0.82, 0));
      g.add(sph(0.1, 12, mat(theme.torchColor, theme.torchColor), 0, 0.82, 0));
    } else if (themeName === 'forest') {
      // Glowing mushroom cluster
      g.add(cyl(0.04, 0.08, 0.3, 10, mat(0x2a3818), 0, 0.2, 0));
      g.add(sph(0.14, 12, mat(theme.torchColor, theme.torchColor, 0.8), 0, 0.42, 0));
    } else {
      // Standard wall torch
  ...
  function buildForestProps(size, theme) {
    ...
    [[-h+1.5,-h+1.5],[h-1.5,-h+1.5],[-h+1.5,h-1.5],[h-1.5,h-1.5]].forEach(([x,z]) => {
      ...
      gridGroup.add(sph(0.5, 12, mat(0x224010,0x0a1808), x+0.2, 2.6, z-0.2));
    });
    // Mushrooms scattered
  ...
  function buildIceProps(size, theme) {
    ...
    [[-2,0,0],[2,0,1],[0,0,-2]].forEach(([x,y,z]) => {
      gridGroup.add(cone(0.2, 0.7, 8, mat(0x88aacc,0x224466,0.9), x, 0.4, z));
      gridGroup.add(cone(0.13, 0.45, 8, mat(0x88aacc,0x224466,0.9), x+0.22, 0.25, z+0.18));
    });
    // Frozen figure
  ...
  function buildThroneProps(size, theme) {
    ...
    gridGroup.add(box(1.12, 0.08, 0.62, mat(0xaa8800,0x664400), tx, 0.38, tz+0.15));
    // Relief on throne back
    gridGroup.add(box(0.72, 1.0, 0.04, mat(0x201010,0x080404), tx, 1.0, tz-0.28));
    gridGroup.add(sph(0.12, 10, mat(0x181018,0x060006), tx, 1.22, tz-0.3)); // skull motif

    // SYMMETRIC ALTAR PEDESTALS — matches reference image (2 rows of 3 on each side)
    [
      [-3.0, 0, -2.5], [-3.0, 0, 0.5], [-3.0, 0, 3.5],
      [ 3.0, 0, -2.5], [ 3.0, 0, 0.5], [ 3.0, 0, 3.5],
    ].forEach(([px, py, pz]) => {
      gridGroup.add(box(0.9,  0.14, 0.9,  mat(0x181220), px, 0.07, pz)); // base
      gridGroup.add(box(0.72, 0.58, 0.72, mat(0x141018), px, 0.36, pz)); // shaft
      gridGroup.add(box(0.86, 0.12, 0.86, mat(0x1e1828), px, 0.71, pz)); // cap
      gridGroup.add(box(0.78, 0.10, 0.78, mat(0x201a2e), px, 0.82, pz)); // top platform
      gridGroup.add(cyl(0.22, 0.16, 0.18, 12, mat(0x282028), px, 0.97, pz)); // flame bowl
      gridGroup.add(cyl(0.10, 0.06, 0.26, 8, mat(0xff4400,0xff2200,0.9), px, 1.12, pz));
      gridGroup.add(sph(0.10, 8, mat(0xff8800,0xff4400,0.8), px, 1.26, pz));
      const pedL = new THREE.PointLight(0xff2200, 1.2, 4.5);
      pedL.position.set(px, 1.4, pz);
      lightGroup.add(pedL);
    });

    // Ornate columns (with base and capital)
    [[-3.5, 0, -4], [-3.5, 0, 0], [-3.5, 0, 4],
     [ 3.5, 0, -4], [ 3.5, 0, 0], [ 3.5, 0, 4]].forEach(([x,y,z]) => {
      gridGroup.add(box(0.62, 0.12, 0.62, mat(0x201610), x, 0.06, z));      // base
      gridGroup.add(cyl(0.2, 0.25, 2.5, 12, mat(0x1a1010), x, 1.3, z));    // shaft
      gridGroup.add(box(0.58, 0.12, 0.58, mat(0x221818), x, 2.6, z));       // capital
  ...
  function buildEnemyModel(typeId, isBoss) {
    ...
    if (isSp) {
      ...
      const mShld  = mat(0x2a2838, 0x100c18); // shield face
      const mShldR = mat(0x888878, 0x303020); // shield rim

      // Ribcage torso
      bodyGrp.add(box(0.44*sc,0.56,0.3,mBone,0,0.60,0));
      for (let r=0;r<4;r++) bodyGrp.add(box(0.38*sc,0.044,0.22,mBoneD,0,0.38+r*0.11,0));
      bodyGrp.add(box(0.08,0.56,0.08,mBoneD,0,0.60,0)); // spine strip

      // Skull
      bodyGrp.add(sph(0.22*sc,12,mBone,0,0.96,0));
      bodyGrp.add(box(0.18*sc,0.1,0.18,mBoneD,0,0.84,0.04)); // jaw
      bodyGrp.add(box(0.1,0.09,0.025,mat(0xcc2200,0x880000),-0.09,0.98,0.20)); // eye socket L
      bodyGrp.add(box(0.1,0.09,0.025,mat(0xcc2200,0x880000), 0.09,0.98,0.20)); // eye socket R
      bodyGrp.add(box(0.06,0.06,0.04,mBoneD,-0.15,0.91,0.18)); // cheekbone L
      bodyGrp.add(box(0.06,0.06,0.04,mBoneD, 0.15,0.91,0.18)); // cheekbone R

      // Leg bones (upper thigh)
      bodyGrp.add(box(0.13*sc,0.52,0.13,mBone,-0.16,0.22,0));
      bodyGrp.add(box(0.13*sc,0.52,0.13,mBone, 0.16,0.22,0));
      // Lower legs
      bodyGrp.add(box(0.1*sc,0.44,0.1,mBoneD,-0.16,-0.24,0));
      bodyGrp.add(box(0.1*sc,0.44,0.1,mBoneD, 0.16,-0.24,0));
      // Feet
      bodyGrp.add(box(0.12*sc,0.08,0.18,mBone,-0.16,-0.50,0.04));
      bodyGrp.add(box(0.12*sc,0.08,0.18,mBone, 0.16,-0.50,0.04));

      // Arms
      bodyGrp.add(box(0.11,0.48,0.11,mBone,-0.34*sc,0.60,0)); // shield arm
      bodyGrp.add(box(0.11,0.48,0.11,mBone, 0.34*sc,0.60,0)); // sword arm

      // SWORD right hand — blade points upward
      const sx = 0.46*sc;
      bodyGrp.add(box(0.04,0.66,0.03,mSword,sx,0.82,0));               // blade
      bodyGrp.add(box(0.24,0.04,0.04,mat(0x887828,0x443808),sx,0.52,0)); // crossguard
      bodyGrp.add(box(0.08,0.18,0.08,mat(0x3c2410,0x1c0e06),sx,0.37,0)); // grip
      bodyGrp.add(sph(0.065,8,mat(0x907830,0x403008),sx,0.27,0));        // pommel

      // SHIELD left hand
      const shx = -0.52*sc;
      bodyGrp.add(box(0.08,0.58,0.46,mShld,shx,0.60,0.07));             // shield body
      bodyGrp.add(sph(0.075,8,mat(0x907830,0x443808),shx,0.60,0.14));   // boss stud
  ...
  function stopOrbit(ev) {
    if (ev.type === 'pointerup' && ev.button !== 2) return;
    orbitActive = false;
    renderer.domElement.style.cursor = '';
  }
  function onOrbitMove(ev) {
    if (!orbitActive || !camera) return;
    const dx = ev.clientX - orbitLastX;
    const dy = ev.clientY - orbitLastY;
    orbitLastX = ev.clientX;
    orbitLastY = ev.clientY;
    // accumulate velocity for inertia (pixels -> rad/s estimate at 60fps)
    orbitVTheta = -dx * 0.007 * 60;
    orbitVPhi   =  dy * 0.005 * 60;
    orbitTheta += -dx * 0.007;
    orbitPhi = Math.max(0.18, Math.min(1.35, orbitPhi + dy * 0.005));
    camera.position.set(
      ORBIT_R * Math.sin(orbitPhi) * Math.sin(orbitTheta),
      ORBIT_R * Math.cos(orbitPhi),
      ORBIT_R * Math.sin(orbitPhi) * Math.cos(orbitTheta)
    );
    camera.lookAt(0, 0, 0);
  }
  ...
  function mount(containerEl, callbacks = {}) {
    ...
    renderer.domElement.addEventListener('pointerdown',  onRightDown);
    ...
    function animate() {
      ...
      if (!orbitActive && camera && (Math.abs(orbitVTheta) > 0.0005 || Math.abs(orbitVPhi) > 0.0005)) {
        ...
        orbitTheta += orbitVTheta * dt;
        orbitPhi = Math.max(0.18, Math.min(1.35, orbitPhi + orbitVPhi * dt));
        orbitVTheta *= decay;
        orbitVPhi   *= decay;
        camera.position.set(
          ORBIT_R * Math.sin(orbitPhi) * Math.sin(orbitTheta),
  ...
  function unmount() {
    ...
    if (renderer) {
      ...
      renderer.domElement.removeEventListener('pointerleave', stopOrbit);
